...
class Pit:
    ...
    Raises:
        ValueError: when ``fcst_type`` is not one of "ensemble" or "cdf".
        ...
        ValueError: when ``fcst_type='cdf'`` and any values of ``fcst`` that are less then 0 or greater than 1.
        ValueError: when ``fcst_type='cdf'``, ``fcst_left`` is not ``None`` and any
            values of ``fcst_left`` are less then 0 or greater than 1.
        ValueError: when ``fcst_type='cdf'``, ``fcst_left`` is not ``None`` and
            any values of ``fcst_left`` are greater than ``fcst``.
        ValueError: when ``fcst_type='cdf'`` and ``fcst[special_fcst_dim]`` is not increasing.

    Warns:
        UserWarning: when ``fcst_type='cdf'``, and any values of ``obs`` are less than
            the minimum of ``fcst[special_fcst_dim]`` or greater than the maximum of
            ``fcst[special_fcst_dim]``.
        UserWarning: when ``fcst_type='cdf'`` and there are any NaN values in ``fcst`` or
            ``fcst_left``.
