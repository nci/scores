...
DA_ADD_THRESHOLDS = xr.DataArray(
    ...
    dims=["date", "station", "x"],
    coords=dict(station=[1001, 1002], date=["2020-01-01"], x=[0, 0.2, 0.5, 1]),
)

EXP_ADD_THRESHOLDS1 = xr.DataArray(
    data=[[[0.2, 0.4, 1, 1, 1], [0, 0, 0.6, 0.8, 1]]],
    dims=["date", "station", "x"],
    coords=dict(station=[1001, 1002], date=["2020-01-01"], x=[0, 0.2, 0.5, 0.75, 1]),
)
...
DA_DECREASING_CDFS1 = xr.DataArray(
    ...
    dims=["station", "x"],
    coords=dict(station=[1001, 1002, 1003, 1004], x=[0, 1, 2, 3, 4]),
)

EXP_DECREASING_CDFS1A = xr.DataArray(
    data=[False, True, True, False],
    dims=["station"],
    coords=dict(station=[1001, 1002, 1003, 1004]),
)
