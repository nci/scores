...
from scores.utils import DimensionError, gather_dimensions
...
def test_gather_dimensions_examples():
    ...
    assert gather_dimensions(fcst_dims, obs_dims, reduce_dims="lat") == set(["lat"])
    assert gather_dimensions(fcst_dims, obs_dims, reduce_dims=["lat", "lon"]) == set(["lat", "lon"])
    assert gather_dimensions(fcst_dims, obs_dims, reduce_dims=["lat", "lat", "lon"]) == set(["lat", "lon"])

    # Tests if reduce_dims and preserve_dims are both None
    assert gather_dimensions(fcst_dims, obs_dims) == fcst_dims

    # Reduce every dimension if the string "all" is specified
    assert gather_dimensions(fcst_dims, obs_dims, reduce_dims="all") == fcst_dims

    # Reduce "all" as a named dimension explicitly
    assert gather_dimensions(fcst_dims_conflict, obs_dims, reduce_dims=["all"]) == set(["all"])

    # Basic tests on preservation
    assert gather_dimensions(fcst_dims, obs_dims, preserve_dims="lat") == set(["base_time", "lead_time", "lon"])
    assert gather_dimensions(fcst_dims, obs_dims, preserve_dims=["lat", "lon"]) == set(["base_time", "lead_time"])
    assert gather_dimensions(fcst_dims, obs_dims, preserve_dims=["lat", "lat", "lon"]) == set(
        ["base_time", "lead_time"]
    )

    # Preserve every dimension if the string "all" is specified
    assert gather_dimensions(fcst_dims, obs_dims, preserve_dims="all") == set([])

    # Preserve "all" as a named dimension explicitly
    assert gather_dimensions(fcst_dims_conflict, obs_dims, preserve_dims=["all"]) == set(
        ["base_time", "lead_time", "lat", "lon"]
    )

    # Test that preserve is the inverse of reduce
    preserve_all = gather_dimensions(fcst_dims, obs_dims, preserve_dims="all")
    reduce_empty = gather_dimensions(fcst_dims, obs_dims, reduce_dims=[])
    ...
    assert gather_dimensions(fcst_dims, obs_dims, reduce_dims="lead_time") == set(["lead_time"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (utils_test_data.DA_B.dims, utils_test_data.DA_R.dims, utils_test_data.DA_G, "blue", None, None, {"blue"}),
        (utils_test_data.DA_B.dims, utils_test_data.DA_R.dims, utils_test_data.DA_G, ["blue"], None, None, {"blue"}),
        (
...
def test_gather_dimensions_more_examples(fcst, obs, weights, reduce_dims, preserve_dims, special_fcst_dims, expected):
    """
...
def test_gather_dimensions_exceptions():
    ...
    with pytest.raises(ValueError):
        gather_dimensions(fcst_dims, obs_dims, preserve_dims=[], reduce_dims=[])

    # Attempt to reduce a non-existent dimension
    with pytest.raises(ValueError) as excinfo:
        assert not gather_dimensions(fcst_dims_conflict, obs_dims, reduce_dims="nonexistent")
    ...
    with pytest.raises(ValueError) as excinfo:
        assert not gather_dimensions(fcst_dims_conflict, obs_dims, preserve_dims="nonexistent")
    ...
    # Preserve "all" as a string but named dimension present in data
    with pytest.warns(UserWarning):
        assert gather_dimensions(fcst_dims_conflict, obs_dims, reduce_dims="all") == fcst_dims_conflict
...
def test_gather_dimensions_more_exceptions(
    fcst, obs, weights, reduce_dims, preserve_dims, special_fcst_dims, error_msg_snippet
):
    """
